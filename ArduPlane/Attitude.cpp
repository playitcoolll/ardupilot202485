...
float Plane::stabilize_roll_get_roll_out()
{
    ...
    demanded_roll_new = 2000 * channel_roll->norm_input_dz();//期望滚转角验证完毕
...
float Plane::stabilize_pitch_get_pitch_out()
{
    ...
    // }
    // hal.console -> printf("%d",channel_pitch->get_control_in());


    //姿态模式
    demanded_pitch_new = 1500 * channel_pitch->norm_input_dz();
    if(demanded_pitch_new <= -700){
        demanded_pitch_new = -700;
    }
...
int16_t Plane::calc_nav_yaw_coordinated()
{
    ...
    float error_ssa =0 + ssa;
    
    float out_ssa = boost_ssa * (error_ssa - degrees(rate_z));

    float commanded_rudder_my = out_ssa + rudder_in;//注意rud方向
