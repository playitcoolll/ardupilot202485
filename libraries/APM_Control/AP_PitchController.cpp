...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode)
{
    ...
    //     ssa_use = 0;
    // }else{
    //     ssa_use = _ahrs.getSSA();
    // }
    // if(abs(ssa_use) <= 0.01){
    //     ssa_use = 0;
    // }
    // float aoa_rad = aoa * M_PI / 180.0;
    // float ssa_rad = ssa_use * M_PI / 180.0;
    // float R[3][3] = {{cosf(aoa_rad)*cosf(ssa_rad), -cosf(aoa_rad)*sinf(ssa_rad), -sinf(aoa_rad)},
    //                  {sinf(ssa_rad),                       cosf(ssa_rad),                     0}, 
    //                  {sinf(aoa_rad)*cosf(ssa_rad), -sinf(aoa_rad)*cosf(ssa_rad),  cosf(aoa_rad)}};体轴系到航迹系转换矩阵
    ...
    if(_ahrs.get_velocity_NED(Ve)){
        ...
        // Vector3f g2ahangji = multiplyMatrixVector2(dcm,g_g);
    ...
    //迎角反馈p
    // float aoa;
    // boost_aoa = 1 * abs(aoa - 0) + 1;
    // constrain_float(boost_aoa , 1, 5);
    ...
    // //计算D
    // D_term = 0.05 * (error_my-degrees(rate_y));


    //姿态反馈pid方法一
    float boost_pitch = 0.1;//飞翼冲浪者0.2太大 高速不行
    //计算P
    error_my = desired_rate-degrees(rate_y);
    float out_my_p = boost_pitch * error_my;

    //计算I
    float ki_pitch = 0.1;//飞翼冲浪者0.2 0.05
    // if(abs(desired_rate/40.0f) <= 0.15){
    //     ki_pitch = 0;
    // }
    if(!disable_integrator){
        if(underspeed){
    ...
    float I_term =  integral;
    // hal.console -> printf("%f\n",I_term);
    I_term = constrain_float(I_term , -100.0F,100.0F);//如果拉杆起不来就是这个地方小了
    hal.console -> printf("%f\n",I_term);
    //计算D
    derivative = (error_my - previous_error) / dt;
    // D_term = 0.03 * derivative;//飞翼0.05 
    ...
    float k_p = 1;
    if(aspeed > 4){
        k_p = 1.0f/(abs(aspeed)+1.0f);//
    }
    // float k_d = 1;
    // if(aspeed > 4){
    //     k_d = 1.0f/(abs(aspeed)+1.0f);//
    // }
    // float k_i = 1;
    // if(aspeed > 4){
    //     k_i = 1.0f/(abs(aspeed)+1.0f);//1 - (0.2/10*(aspeed - 4))
    // }//速度大要给高吗？？


    float out_my = out_my_p * k_p + I_term * k_p + D_term * k_p + out_aoa * k_p + out_az * k_p;//
    ...
    // 法一飞翼冲浪者
    // float k_out = 1.0f/(abs(aspeed)+1.0f);
    float k_out = 1.0f;
    // hal.console -> printf("%f\n",k_out);
    // k_out = constrain_float(k_out,0.04,0.2 );
    out_my = k_out * out_my;
    // out_my =  out_my;
    //测试输出的时候一定用下面的，不然当静止状态的时候输出为inf
    // out_my = k_out*out_my;
    // hal.console -> printf("%f\n",out_my*100);
...
float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    // // 方法二角度误差
    // angle_err_deg = angle_err * 0.01;
    // float desired_rate = angle_err_deg * 2;
