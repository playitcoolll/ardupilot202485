...
float AP_RollController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    //计算P
    float boost_roll = 0.5;

    error_my = desired_rate-degrees(rate_x);
    float out_my_p = boost_roll * error_my;
    ...
    integral += ki_roll * error_my * dt;
    float I_term =  integral;


    //计算D
    derivative = (error_my - previous_error) / dt;
    // D_term = 0.05 * derivative;//飞翼0.05
    D_term = 0.01 * derivative;//冲浪者

    float out_my = out_my_p + I_term + D_term;
    ...
    // 求和
    out_my = out_my_p + I_term + D_term ;



    if (underspeed) {
        //低速减小D和P
        out_my -= D_term + 0.5 * out_my_p + I_term;
    }
    ...
    float k_out = 1;
    // if(aspeed > 15){
    //     k_out = 1 - (0.15/10*(aspeed - 15));
    // }
    out_my = (k_out / (abs(aspeed)+1.0f)) * out_my;
...
float AP_RollController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    float desired_rate = angle_err_deg * 20;
